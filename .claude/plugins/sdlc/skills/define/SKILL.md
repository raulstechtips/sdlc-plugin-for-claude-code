...
6. **Write draft** — Use guide's draft body template. Write to `.claude/sdlc/drafts/<level>-<name>.md` (new) or `.claude/sdlc/drafts/<level>-<issue-number>.md` (reshape). Frontmatter MUST include type, name, priority, areas, status, and parent references (parent-epic, parent-feature where applicable). Reshapes MUST include a `## Changes` section documenting what was modified. Ensure `.claude/sdlc/drafts/` directory exists.
