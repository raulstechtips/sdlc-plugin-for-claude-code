...
| 2026-03-31 | TDD is non-negotiable — core pillar of the plugin | Users who don't want TDD should use a different tool; TDD rules are always generated by sdlc:rules regardless of user preferences | sdlc:rules, Architecture |
| 2026-03-31 | Dynamic research over stored baselines for rules generation | Can't maintain best-practice libraries for every language/framework; live research agents stay current and adapt to any stack | sdlc:rules, Research Agents, Reference Guides |
| 2026-03-31 | Reference guides as research instructions, not stored content | Keeps the plugin lightweight and stack-agnostic; guides define what to extract per technology category, not the research itself | sdlc:rules, Reference Guides, Agents |
| 2026-03-31 | sdlc:rules is a sibling to sdlc:init (bootstrapping concern, not artifact lifecycle) | Rules generation is about setting up AI guardrails for the repo, not managing work items; it complements but does not overlap with sdlc:define | Architecture, Skill Inventory |
